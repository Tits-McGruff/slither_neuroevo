//! Emit retained Stage 4 combined Rust sensing/inference evidence.

use slither_native::engine::control_fixture::{
    run_stage4_control_evidence, Stage4ControlEvidenceOptions,
};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "wrote {scenario} combined control evidence to {}",
        output_path.display()
    );
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn warmed_p0_sensing_inference_and_commit_do_not_allocate() {
        let report = run_stage4_control_evidence(
            Stage4ControlEvidenceOptions {
        ...
        assert_eq!(
            report
                .result
                .allocator_operations_per_sensing_and_inference
                .nonzero_passes,
            0
        );
        assert_eq!(
            report.result.allocator_operations_per_commit.nonzero_passes,
            0
        );
    }
}
