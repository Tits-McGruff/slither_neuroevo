...
| Graph definition, ports, offsets and parameter count | `src/mlp.ts::buildArch/enrichArchInfo`, `src/brains/graph/compiler.ts`, `src/brains/stackBuilder.ts` | graph unit/integration tests; `scripts/stage2/graph-baseline.ts`; exact retained default and adversarial fixtures under `scripts/stage2/graph-fixtures/`; paired Windows/Oxygen v2 artifacts; behavior artifact | Preserve graph/port/weight layout compatibility. Correct locale-dependent implicit ordering with an explicit legacy migration order. The owner database's default Hall-of-Fame architecture is covered; its large MLP architecture is currently retained only as an architecture key, so its exact graph replay fixture remains open. |
