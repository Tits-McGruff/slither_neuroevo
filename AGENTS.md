...
At the top level, `package.json` and `package-lock.json` define the Node toolchain (`vite` for dev/build/preview and `vitest` for tests), and `vite.config.mjs` contains the non-default cache directory that avoids file-lock issues on network drives. Windows users have a convenience launcher in `play.bat` that installs dependencies and runs the dev server. Active planning notes live in `todo/phase-*.md` plus `todo/multiplayer-plan.md`, while historical plans are archived under `todo/archive/`. CI is wired through `.github/workflows/node.js.yml`, which runs `npm ci`, `npm run build`, and `npm test` across multiple Node versions.
...
The settings system is in `src/settings.ts`, which constructs grouped sliders from `SETTING_SPECS` and uses `data-path` attributes to map slider values into `CFG` via `setByPath` from `src/utils.ts`. Sliders marked `requiresReset` only apply on world reset; live sliders call back to `src/main.ts`, which posts incremental updates to the worker. The top-level core sliders (snake count, sim speed, layer counts, neuron sizes) are wired directly in `src/main.ts` and must stay aligned with `buildArch()` in `src/mlp.ts` and defaults in `src/config.ts`. Stack order controls and graph spec tooling live in the Settings tab: the `useMLP` + memory toggles define the linear stack, quick layouts just set those toggles, and the Advanced graph section is for custom splits/skips. Saved presets load from the server DB, and the active graph can be copied/exported.

Visualization helpers live in `src/BrainViz.ts`, `src/FitnessChart.ts`, and `src/chartUtils.ts`. The Brain Visualizer renders activation heat strips when the worker sends `stats.viz` data (enabled via the Visualizer tab; `src/main.ts` posts a `viz` message to toggle streaming). In server mode, viz streaming is not available yet. The Stats tab uses a chart selector to render fitness, species diversity, and network complexity from `fitnessHistory` entries sent by the worker or server (min/avg/max plus species/complexity metrics).
