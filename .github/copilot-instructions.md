...
`index.html` defines the tabbed control panel (Settings, Visualizer, Stats, Hall of Fame) plus the God Mode log panel. `styles.css` implements the panel layout, sliders, tab buttons, and simple entry transitions. `src/main.js` wires these DOM elements to the worker, holds a `currentFrameBuffer`, and uses a `proxyWorld` to expose minimal world-like methods (`toggleViewMode`, `resurrect`) to the UI and Hall of Fame code. The God Mode interactions (click to select, right-click to kill, drag to move) depend on parsing the buffer and converting screen coordinates to world coordinates using the camera values embedded in the frame header.
...
Visualization helpers live in `src/BrainViz.js`, `src/FitnessChart.js`, and `src/chartUtils.js`. The Brain Visualizer renders activation heat strips when the worker sends `stats.viz` data (enabled via the Visualizer tab; `main.js` posts a `viz` message to toggle streaming). The Stats tab uses a chart selector to render fitness, species diversity, and network complexity from `fitnessHistory` entries sent by the worker (min/avg/max plus species/complexity metrics).
...
Import/export is exposed in the Settings tab and now syncs through the worker. `src/main.js` requests an export payload from the worker, adds HoF data, and downloads a JSON file. Imports validate the JSON, update the Hall of Fame store, persist population JSON in localStorage, and send the genomes to the worker for an in-place reset.
